	<div id="ubuntuir_header">
		<a id="ubuntuir_logo" href="https://ubuntu.ir/"><img src="https://static.ubuntu.ir/images/logo.png" /></a>
		<div id="ubuntuir_header_navbar">
		<ul>
			<li <? if ($tpl["page"] == "main") { ?>class="ubuntuir_header_active"<? } ?>><a href="http://ubuntu.ir">اوبونتو</a></li>
			<li><a href="http://forum.ubuntu.ir">انجمن‌های پشتیبانی</a></li>
			<li <? if ($tpl["page"] == "wiki") { ?>class="ubuntuir_header_active"<? } ?>><a href="https://wiki.ubuntu.ir">ویکی فارسی</a></li>
			<li <? if ($tpl["page"] == "videos") { ?>class="ubuntuir_header_active"<? } ?>><a href="https://videos.ubuntu.ir">ویدیوها</a></li>
			<li <? if ($tpl["page"] == "events") { ?>class="ubuntuir_header_active"<? } ?>><a href="https://events.ubuntu.ir">همایش‌ها</a></li>
			<li <? if ($tpl["page"] == "irc") { ?>class="ubuntuir_header_active"<? } ?>><a href="http://ubuntu.ir/irc">کانال IRC</a></li>
			<li><a href="http://ubuntu.ir/telegram">گروه Telegram</a></li>
			<li <? if ($tpl["page"] == "pastebin") { ?>class="ubuntuir_header_active"<? } ?>><a href="https://paste.ubuntu.ir">سرویس Pastebin</a></li>
		</ul>
		</div>
	</div>
	<div id="ubuntuir_body">
		<div id="ubuntuir_body_text">
